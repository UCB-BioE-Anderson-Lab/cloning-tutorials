...
window.Deck.sequence("op-kinase", function(slide){
  ...
  go();
  return { steps:[{
    note:"Cast your mind back to the mechanism at the start of the lecture. The hydrolases ran a hydroxyl at a phosphorus and destroyed a bond. This is the same attack with the opposite outcome: the bond is moved, not broken, and it is drawn with the same arrows. Here is why you care. A synthetic oligo arrives with a bare five prime hydroxyl, and you have just seen what happens when that phosphate is missing: ligase has nothing to seal. So you put it on yourself, with polynucleotide kinase and ATP. Follow the arrows. The oligo's own five prime hydroxyl attacks the gamma phosphorus, the same arrow we drew for hydrolysis, only the nucleophile is an alcohol instead of water. The second arrow breaks the ester to beta, and the electrons land on the bridging oxygen, which is what makes ADP the leaving group. Then the product: the phosphate is simply on the DNA. Alpha and beta are labelled only so you can see which one gamma is. And notice the backbone is red and the bases grey, as always: PNK needs DNA and does not read a word of it.",
