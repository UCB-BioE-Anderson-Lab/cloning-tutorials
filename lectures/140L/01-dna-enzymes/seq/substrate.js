...
window.Deck.sequence("substrate", function(slide){
  ...
  const S=[
    ...
    { word:"the ends", code:"GAATTC", sub:"a phosphate, a bare hydroxyl, a biotin, all of them still GAATTC",
      ...
      note:"Third, and the one students forget exists, because it is invisible if you only write the letters. It applies to linear DNA only: a circle has no ends, so it has no end chemistry at all. On a linear molecule every one of these is GAATTC, and what changes is what sits on each five prime end, in red, with the two strands independent of one another. A bare hydroxyl, or a phosphate. A rule of thumb worth carrying: a hydroxyl is how a synthesised oligo arrives, because that is what comes off the synthesiser, and a phosphate is what you find once an enzyme has been at it. End chemistry is a record of where that DNA has been, and it decides whether a ligase can act, whether an exonuclease will start, and whether your PCR product will clone. It does not stop at those two. A synthetic five prime end is built chemically, so you can hang almost anything off it: the last ones here carry a biotin through a linker, for pulling one strand out on streptavidin beads, and dyes go on the same way. Notice biotin never touches the DNA itself. Same sequence every time, every one a different molecule.",
