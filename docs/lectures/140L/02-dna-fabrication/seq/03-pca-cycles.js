...
{ s: st({ y0:350, y1:400, y2:560, y3:610, t1:1 }),
  ...
  bullets:[],
  note:"This is the beat that almost every drawing of this reaction gets wrong, so watch what does not happen. The oligos do not assemble into the full-length molecule. They find partners two at a time, one with two and three with four, and what you have is two separate pieces with a wide gap between them. The full-length molecule does not exist yet and it cannot, this round. The red ticks mark the only sequence each pair actually shares, which is the only reason those two and not some other two.",
...
{ s: st({ y0:350, y1:400, y2:560, y3:610, t1:1, e1:1 }),
  ...
  bullets:[],
  note:"Every 3-prime end in both pairs is recessed, with template running on ahead of it, which is exactly what a polymerase wants. So it extends all of them, and the dashed blue is what it made rather than what you bought. Notice that the solid ink in each strand is still precisely the oligo you ordered, and it will be for the rest of this slide. A strand is founded by one oligo and only ever grows at its 3-prime end.",
...
{ s: st({ y0:310, y1:640, y2:545, y3:400, e1:1 }),
  ...
  bullets:[],
  note:"Melt it, and you are back to four single strands. Count them: the same four, not new molecules. The only thing that changed in the first round is that every one of them is longer than it was.",
...
{ s: st({ y0:340, y1:630, y2:580, y3:390, e1:1, t2:1 }),
  ...
  bullets:[],
  note:"Four strands make exactly two cross-pairs, and both of them form. Now look at where the 3-prime ends are sitting, because that is the only difference between the two. In the top pair both 3-prime ends have template underneath them. In the bottom pair the two strands are held by exactly the same homology, but both 3-prime ends finish out at the corners with nothing under them at all. Same sequence, same annealing, and one of these two is a dead end.",
...
{ s: st({ y0:340, y1:630, y2:580, y3:390, e1:1, e2:1, t2:1 }),
  ...
  bullets:[],
  note:"The top pair runs out to the ends of its template and that molecule is now the full length of the target. The bottom pair does not move a base, because neither 3-prime end had anything to copy. That is why this is a cycled reaction rather than a single pass: a good fraction of what anneals each round goes nowhere, and you get back the strands and try again.",
...
{ s: st({ y0:430, y1:630, y2:580, y3:480, e1:1, e2:1, oth:0, mark:1 }),
  ...
  bullets:[],
  note:"There is the target, end to end, two rounds after the program started. Look at what it is made of. A short piece of ordered oligo at each end, and blue everywhere else. Almost none of this molecule was bought. The polymerase wrote it, and the only thing that told it what to write was the overlaps you designed.",
