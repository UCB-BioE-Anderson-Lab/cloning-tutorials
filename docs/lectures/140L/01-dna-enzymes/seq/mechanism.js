...
{ cap:"First, the bond it all happens to",
  ...
  note:"I said a moment ago that I am doing this so that you can read the catalogue, and that is true, but it is not the only reason. This is a lecture about biochemistry, and I want you to understand the tools you are using down to the atom. The whole day comes down to three chemical reactions. Two of them are the attack I mentioned a minute ago, a hydroxyl onto a phosphorus, and they differ only in what is doing the attacking; the third is on a different atom altogether and is the odd one out. And the first thing to say about all three is that there is nothing special about them. They happen on their own, in water, with no protein anywhere near them — just extremely slowly. What an enzyme does is take one of those reactions and accelerate it enormously. But it does not stop there, and this is the part that matters for the rest of the lecture: it also restricts it. To one sequence, or one kind of end, or one position on one base. The reaction is ordinary. The restrictions are what you are actually buying. There are only a few hundred common reaction mechanisms in the whole of biology, and every one of them has this kind of nuance to it. The reason we can be as precise as we are about the DNA-modifying ones is, frankly, commercial: there is money in selling these enzymes, so they have been characterised in enormous detail. Most of biology has not had that attention. That does not mean the complexity is not there — it means nobody has mapped it yet. But the starting point is the same everywhere, and it is why I want to begin here: everything that happens in a cell corresponds to a real chemical reaction, one that would happen anyway if you waited long enough.",
...
{ cap:"1 &nbsp;Water attacks the phosphorus",
  ...
  note:"So here is the first of the three, hydrolysis, one step at a time and with no enzyme anywhere — this is the uncatalysed reaction, just water. Think about what is actually happening in the tube. Everything is jostling about, colliding millions of times a second, and essentially all of those collisions do nothing at all. Every so often a water hits the phosphorus at just the right angle, with a lone pair pointing the right way, and those two electrons can drop into an empty orbital on the phosphorus. That angle is not arbitrary. It is straight down the line of the bond that is going to break, coming in from the opposite side, because the orbital they are dropping into is the antibonding orbital of that very bond — put electrons in it and the bond weakens. That is why the attack has to be on the axis. And notice what is attacking. It is water. Not hydroxide. At pH seven there is one ten-millionth molar hydroxide, so essentially nothing is waiting around as hydroxide — the water attacks as water, and the proton problem gets solved afterwards. The arrow starts on a lone pair, because that is what is moving, and ends at the phosphorus, because that is where those two electrons end up.",
...
{ cap:"2 &nbsp;Now that oxygen has three bonds",
  ...
  note:"The bond has formed, so now count. That oxygen has three bonds to it: two hydrogens and the phosphorus. Three bonds on oxygen means a positive charge, and there it is. Phosphorus is holding five things at once, arranged as a trigonal bipyramid, and that is the only moment in the whole reaction when it does. Nothing has left yet. Now the proton comes off, and this is the part people skip: it does not just evaporate. A neighbouring water takes it. Two arrows, and both matter. The first starts on a lone pair of that second water and ends at the hydrogen — that is the water grabbing the proton. The second starts on the oxygen-hydrogen bond itself and ends on the oxygen, because when the proton leaves it leaves its bonding electrons behind.",
...
{ cap:"3 &nbsp;The bond to the leaving sugar breaks",
  ...
  note:"The proton is gone and it is now on that neighbouring water, which is hydronium, positive, sitting there. The attacking oxygen is a neutral hydroxyl on the phosphorus. Now the leaving group goes. Two arrows again. One starts on a lone pair of that anionic oxygen and ends on the phosphorus-oxygen bond, remaking the double bond. That push is what displaces the sugar: the other arrow starts on the bond from phosphorus to the leaving oxygen and ends on that oxygen, so the sugar leaves taking both electrons, as an alkoxide.",
...
{ cap:"4 &nbsp;The alkoxide takes a proton back",
  ...
  note:"Last step, and it is bookkeeping catching up. The sugar left as an alkoxide, which is a strong base, and there is hydronium sitting right there. So it takes a proton straight back. Same two arrows as before, run the other way: a lone pair on the alkoxide reaches for the hydrogen, and the bond that hydrogen was using collapses back onto its oxygen. Follow the proton across all four steps and you will see something neat — the proton the attacking water brought with it ends up on the sugar that left. The water network just moved it around.",
...
{ cap:"The bond is gone, and the pieces are capped",
  ...
  note:"And there are the products. One piece keeps the phosphate and has gained a hydroxyl on it. The other piece is a sugar with a free hydroxyl. The water that attacked is now part of the phosphate, and its proton is on the other fragment. That is what every nuclease and every phosphatase in this lecture is doing: a phosphate end and a hydroxyl end, and the bond between them is gone, not moved. And it is nearly always the same bond. The one that breaks is between the three prime oxygen and the phosphorus, which is why the phosphate is left behind on the downstream piece — a five prime phosphate on one side of the break and a three prime hydroxyl on the other. Hold on to which end gets which, because that single fact is what decides whether the pieces can be put back together later.",
...
{ cap:"Now run it again with an alcohol",
  ...
  note:"Same film, one substitution. Swap the water for an alcohol — here a sugar's three prime hydroxyl, with the ring greyed out because the ring is not the point. It comes in on the same axis, directly opposite the bond that breaks, with the same lone pair and the same arrow. Every step you just watched happens again exactly as it did, including the proton bookkeeping: this oxygen will pick up a positive charge when it has three bonds, and lose its proton to the surrounding water. I am not going to draw those four frames again. Watch what is different at the end.",
...
{ cap:"The phosphate is handed on, not released",
  ...
  note:"And there is the difference, and it is the whole reason this lecture is ordered the way it is. With water, the phosphate ended up on water and the bond was destroyed. With an alcohol, the phosphate ends up on the alcohol — it has been handed from one sugar to another. The bond has moved rather than gone. That attacking alcohol is a three prime hydroxyl in a polymerase or a ligase, a five prime hydroxyl in a kinase, a serine or a tyrosine on the protein itself in a recombinase. Identical chemistry, opposite outcome. Do not let the similarity fool you into thinking a nuclease and a ligase are doing the same thing. And that is two of the three. The third has no phosphorus in it at all — it is a methyl handed from one molecule to another — and it is different enough that it can wait for the enzymes that do it, at the end of the day.",
