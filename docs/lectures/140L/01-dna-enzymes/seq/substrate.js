...
window.Deck.sequence("substrate", function(slide){
  ...
  const S=[
    ...
    { word:"the substrate", sub:"one molecule &mdash; and every part of it has a name",
      ...
      note:"Now the substrate, because an enzyme can be picky in four separate ways and we are going to take them one at a time. The first is the simplest: which part of the molecule it touches at all. So here is the molecule — two strands, antiparallel, every base paired. Name the parts once, carefully, because there are only five places on this molecule where chemistry ever happens, and every enzyme in the lecture is defined by which of them it touches. The sugar — and note there is no hydroxyl at the two prime position, which is what makes it deoxyribose rather than ribose. The phosphate, joining one sugar to the next. The base, hanging off each sugar. And then the two that everyone mixes up: the free hydroxyl at the five prime end and the free hydroxyl at the three prime end. Those two are not interchangeable, and you will be asked to tell them apart all day. Here is how I do it quickly, and it works on every drawing in this deck. Look at the five prime end: the oxygen is not on the ring. There is a carbon in between, so the bond takes a little elbow on its way out to the hydroxyl. The three prime hydroxyl has no elbow — its oxygen sits straight on the ring. Elbow, five prime. No elbow, three prime. Five things, and everything for the rest of today is one enzyme choosing among them.",
    ...
    { word:"degeneracy", sub:"red is required &mdash; grey could be anything",
      ...
      note:"Now the second thing an enzyme can care about: which positions it actually reads. Red is required, grey is free. This is the same six bases as the slide before, but read it as a pattern rather than a sequence: G at the first position, A at the second, then anything, then anything, then T, then C. The enzyme needs G-A at the start and T-C at the end, and the two in between can be whatever they like. Underneath you can see both — the site as you would write it, G-A-N-N-T-C, and below it whatever is actually there this second, changing as we watch. Every one of those is a real base; there is no such thing as an N in a tube. And notice the whole backbone is red, every sugar and every phosphate, because the enzyme does require the DNA to be there and to be continuous. What is free is the identity of those two bases, and nothing else.",
    ...
    { word:"the ends", code:"GAATTC", sub:"a phosphate, a bare hydroxyl, a biotin &mdash; all of them still GAATTC",
      ...
      note:"Third axis, and it is the one students most often forget exists, because it is invisible if you only write the letters. Start with the obvious: this applies to linear DNA only. A circle has no ends at all, so it has no end chemistry, and that on its own decides what a good many enzymes will do to it. On a linear molecule, every one of these is GAATTC and what changes is what sits on each five prime end, marked in red, with the two strands independent of one another. A bare hydroxyl, or a phosphate. There is a rule of thumb worth carrying out of here: a free hydroxyl is actually rare in biology, but it is exactly how a synthesised oligo arrives, because that is what comes off the synthesiser — whereas a phosphate is generally what you find once an enzyme has been at it. So the end chemistry is a record of where that DNA has been. And the difference decides whether a ligase can act, whether an exonuclease will start, and whether your PCR product will clone. It does not stop at those two either. The five prime end of a synthetic oligo is built chemically, so you can hang essentially anything off it: the last ones here carry a biotin through a linker, which is how you would pull one strand out on streptavidin beads, and dyes go on exactly the same way. We will meet some of that later in the course. Notice biotin never touches the DNA itself. Same sequence every time, and every one a different molecule.",
