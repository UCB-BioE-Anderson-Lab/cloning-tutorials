...
window.Deck.sequence("op-phosphatase", function(slide){
  ...
  function paint(g,gb,ga){
    svg.innerHTML=
      ...
      internal(800,660)+
      label(800,806,"an internal phosphate is a diester in a chain, not a substrate",RED,26);
  }
  function go(){
  ...
  go();
  return { steps:[{
    note:"Read the first line literally, because it is more useful than it looks. An alkaline phosphatase does not recognise DNA. It recognises a phosphate ester on the end of something, and does not care what the something is. Here are four unrelated molecules: the five prime end of a DNA, the five prime end of an RNA, a free dNTP, and p-nitrophenyl phosphate, which is not a nucleic acid at all. Every terminal phosphate on all four comes off. That last one is why the same enzyme doubles as the colorimetric reporter in an ELISA: strip the phosphate off pNPP and what is left goes yellow. Now the one at the bottom, which is the important one. That is a phosphate too, and it is untouched, because it sits between two sugars in the middle of a backbone. A terminal phosphate is a monoester and it is the substrate; a phosphate in a chain is a diester and it is not. That distinction is the whole specificity of the enzyme, and the consequence is why we bother: a five prime end with no phosphate cannot be ligated, and lambda exonuclease will not start on it either.",
