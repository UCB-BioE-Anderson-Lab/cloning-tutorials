...
const ECORI = {
  ...
  note:"This is the operator for the whole class, and every enzyme section from here starts with one of these. Read it like this. Red is what the enzyme requires; grey is what it does not care about. The whole backbone is red, every sugar and every phosphate, because an endonuclease needs DNA that is actually there and actually continuous. That is true of all of them. The six bases are red too, because this one is EcoRI and it genuinely requires G-A-A-T-T-C; the pairs on either side are grey because it does not care what surrounds the site. Not every endonuclease has red bases at all. DNase I is coming later in this section and cuts anywhere. Now watch the reaction. One strand is cut. Then the other. It does not happen in one instant, and the two cuts are not opposite each other: they are four base pairs apart. And look at the chemistry left behind, because it never varies: a five prime phosphate on one side, a three prime hydroxyl on the other.",
