...
const FR = [
  { on:["target"],
    ...
    note:"Start with the strain you actually want to change, and note what is not required: no att site, no engineered landing pad, nothing. That is the difference between this section and the last one. Phage integration puts your DNA where the phage chose. This puts it where you choose, and the only requirement on the strain is that it has the sequence you are aiming at.",
  ...
  { on:["target","arms","kd46","tmpl","oligos","pcr"],
    ...
    note:"What comes out is one linear double-stranded DNA, and it is worth looking at what it is made of. The middle is the marker, straight off the plasmid, identical every time. The two ends are forty bases of the genome each, and they are the entire design. Every knockout anybody does this way is the same molecule with a different forty bases on each end.",
...
const MK = [
  ...
  { on:["cp20"], s:{u:0},
    ...
    note:"A second helper plasmid, pCP20, carrying Flp. Same trick as pKD46 before it: a temperature-sensitive origin, so once it has done its job you can grow the cells warm and be rid of it. Two helper plasmids in one experiment and neither of them ends up in the strain, which is the point of the whole exercise.",
